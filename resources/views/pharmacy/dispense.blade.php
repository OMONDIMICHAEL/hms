<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto p-6">
        <div class="bg-white rounded-xl shadow-md overflow-hidden p-8">
            <div class="mb-8">
                <h2 class="text-2xl font-bold text-gray-800 mb-2">Dispense Medication</h2>
                <p class="text-gray-600">Fill out the form below to dispense medication to the patient</p>
            </div>

            <form action="{{ route('dispense.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="prescription_id" required
                            ...
                            placeholder="Enter prescription ID" value="{{ $record->id }}">
                ...
                <div>
                    ...
                    <select name="stock_item_id" required
                        ...
                        @foreach($stockItems as $item)
                            <option value="{{ $item->id }}">{{ $item->medicine_name }} ({{ $item->quantity }} in stock) - Batch #{{ $item->batch_number }}</option>
                        @endforeach
                    </select>
                </div>
